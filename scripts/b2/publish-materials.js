...
const contentDir = path.join(projectRoot, 'content', 'b2');
const outputDir = path.join(projectRoot, 'output', 'b2', 'materials');
const portalDir = path.join(projectRoot, 'portal', 'downloads', 'b2');
...
for (const { slug, group, source } of materials) {
    for (const variant of ['student', 'teacher']) {
        ...
        run(
            process.execPath,
            ['scripts/build.js', sourceFile, '-b', 'b2', '-v', variant, '-o', rawPdf],
            `Build ${slug} (${variant})`
        );
...
console.log('B2 materials published successfully.');
