#!/usr/bin/env node
// Publish B1 materials to portal/downloads/b1/ as student/teacher PDFs.
// Modeled on scripts/b4/publish-materials.js; also covers the
// complementary-material files (review worksheet + end-of-level test),
// which for B4 were built with scripts/build.js directly.
...
const contentDir = path.join(projectRoot, 'content', 'b1');
const outputDir = path.join(projectRoot, 'output', 'b1', 'materials');
const portalDir = path.join(projectRoot, 'portal', 'downloads', 'b1');
...
for (const { slug, group, source } of materials) {
    for (const variant of ['student', 'teacher']) {
        ...
        run(
            process.execPath,
            ['scripts/build.js', sourceFile, '-b', 'b1', '-v', variant, '-o', rawPdf],
            `Build ${slug} (${variant})`
        );
...
console.log('B1 materials published successfully.');
