...
One printable exam, four graduated sections, strictly in ladder order. Objective items only — every item is 1 point, right or wrong, no partial credit. No open writing, no listening, no speaking in the scored instrument.

```text
Section A — B1 material (10 points)
...
Section B — B2 material (10 points)
...
Section C — B3 material (10 points)
...
Section gate: **7/10 (70%)**. Floor gate: **4/10 (40%) on Section A**. The coordinator scores sections in order and applies the first matching row:

```text
Section A below 4              →  B0  (absolute beginner — see B0 caveat)
Section A below 7              →  B1
A ≥ 7, Section B below 7       →  B2
A ≥ 7, B ≥ 7, C below 7        →  B3
A ≥ 7, B ≥ 7, C ≥ 7, D below 7 →  B4
All four sections ≥ 7          →  beyond B4 — coordinator conversation
...
- Student info box + score table (`vocab-table` pattern): one row per section `____ / 10`, total `____ / 40`, plus a **Placement** result line the coordinator fills from the rubric.
- Instruction callout: PT-first, includes the stop rule.
- Four sections as `unit` blocks (`page-break-before` between sections), ~10 items each, mission-context illustration at the top (mandatory per AGENTS.md).
...
2. New student sits the exam before Class 1; coordinator reads the PT instruction aloud; 40 minutes; student stops where the stop rule bites.
...
5. Score of exactly 6 on the deciding section → runs the 5-minute conversation check → places, defaulting down.
...
4. All items objective, 1 point each, 10 per section; tolerant answer keys where multiple forms are correct (existing precedent: "accept any clear grouping…").
